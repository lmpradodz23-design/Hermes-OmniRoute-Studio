...
Map `AskUserQuestion` to the available user-input mechanism, or infer reversible defaults when the user explicitly authorized autonomous work. Map `ai-artist` and `ai-multimodal` to Hermes' configured image-generation tool. Map `chrome-devtools` to the embedded browser, preview, or Playwright screenshot capability. Never execute the upstream `.claude/skills/...` example paths directly.
